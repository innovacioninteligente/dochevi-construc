...
export const budgetSearchAgent = ai.defineFlow(
    {
        ...
        inputSchema: z.object({
            ...
            context: z.string().optional().describe('Context to boost semantic match relevance (e.g. Chapter Name)'),
        }),
    ...
    async (input) => {
        ...
        if (input.intent === 'BOTH' || input.intent === 'PARTIDA' || (input.intent === 'MATERIAL' && input.generic_query)) {
            ...
            if (input.intent !== 'MATERIAL') {
                pbResult = await priceBookRetrieverTool({ query: input.query, limit: 3, year: 2024, context: input.context });
            }
            ...
            if ((!pbResult.items || pbResult.items.length === 0) && input.generic_query) {
                ...
                pbResult = await priceBookRetrieverTool({ query: input.generic_query, limit: 3, year: 2024, context: input.context });
            }
