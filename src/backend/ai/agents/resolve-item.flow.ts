...
import { budgetSearchAgent } from './budget-search.agent';
    ...
    async (input) => {
        ...
        if (decision.tool === 'budgetSearchAgent') {
            ...
            if (result.partida && result.material) {
                ...
                const { constructionAnalystAgent } = await import('./construction-analyst.agent');
                ...
                const targetLeadId = input.leadId || (await import('@/backend/ai/context/genkit.context')).getCurrentContext()?.userId;
            ...
            if (result.partida) {
                // ... emit event ...
                const { emitGenerationEvent } = await import('@/backend/budget/events/budget-generation.emitter');
                const { getCurrentContext } = await import('@/backend/ai/context/genkit.context');
